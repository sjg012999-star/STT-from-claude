...
- Platform API 키는 기본적으로 `gpt-transcribe` 전사에만 사용합니다. 기존 `gpt-4o` alias는 명시적으로 선택하는 legacy fallback으로 유지합니다.
...
```bash
stt transcribe sample.wav --profile seminar --provider gpt-transcribe --output out/seminar
```
...
서비스 항목을 자동으로 읽습니다. 다른 컴퓨터나 별도 서비스 실행 환경에서는
`OPENAI_API_KEY` 또는 같은 이름의 Keychain 항목을 별도로 구성해야 합니다.
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --ocr-images --output out/seminar
```

긴 녹음 파일 분할 전사:

```bash
stt run long-seminar.wav --profile seminar --provider gpt-transcribe --chunk-audio --chunk-seconds 600 --output out/seminar
```

`--chunk-audio`는 ffmpeg segment 기능으로 `audio_chunks/chunk_*.wav`를 만든 뒤 각 chunk를 전사하고, 타임스탬프를 chunk offset만큼 보정해 하나의 `transcript.md/json/srt`로 합칩니다. OpenAI 파일 업로드 제한에 걸릴 수 있는 긴 Zoom H1e WAV를 `gpt-transcribe`, `gpt-4o`, `gpt-4o-mini`, `whisper-1`로 처리할 때 사용합니다.
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --extract-pdfs --pdf-extractor-script /path/to/extract-figures-tables.py --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --plan-reference-search --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --lookup-references --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --additional-research-file ./research/open-web-notes.md --enrich-notes --llm-summarize --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --web-research-query "InTesTiny RGD nanoparticle" --web-research-endpoint "https://your-search-endpoint.example/api" --enrich-notes --llm-summarize --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --ocr-images --preprocess --chunk-audio --correct --summarize --llm-summarize --lookup-references --enrich-notes --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --pack ./materials --correct --correction-chunk-size 40 --correction-overlap 3 --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --correct --save-glossary ./glossary.tsv --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --correct --lookup-references --write-review-queue --output out/seminar
```
...
```bash
stt run sample.wav --profile seminar --provider gpt-transcribe --correct --save-glossary ./glossary.tsv --review-decisions-file ~/Downloads/review_decisions.json --output out/seminar-reviewed
```

직접 만든 용어 힌트 파일만 추가:

```bash
stt transcribe sample.wav --profile seminar --provider gpt-transcribe --terms-file terms.txt --output out/seminar
```
...
```bash
stt bakeoff sample.wav --profile seminar --providers gpt-transcribe,gpt-4o,gpt-4o-mini,gemini-audio --output out/bakeoff
```
