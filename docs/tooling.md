...
`src/stt_pipeline/reference_lookup.py` creates deterministic DOI/Crossref/OpenAlex/Semantic Scholar lookup plans and can, behind `--lookup-references`, perform metadata lookup plus open PDF caching. Tests use fake HTTP clients; do not add live network tests to the default suite.
