...
Live OpenAI STT is wired behind `OpenAiSttTranscriber`, optional OpenAI transcript correction is wired behind `OpenAiTranscriptCorrector`, optional slide-photo OCR is wired behind `OpenAiSlideImageOcr`, reference lookup planning is deterministic behind `--plan-reference-search`, optional PDF figure/table extraction execution is wired behind explicit CLI flags, and deterministic source-separated notes are wired behind `--enrich-notes`. Tests still use fakes and do not call paid or network APIs. Live web search/download and LLM-based rich summarization are not wired yet.
...
2. Add LLM-based rich summarization behind explicit flags after source-label validation exists.
4. Add skipped integration tests for any optional external tool before wiring it into the CLI.
5. Add local `mlx-whisper` fallback only after adapter tests exist.
