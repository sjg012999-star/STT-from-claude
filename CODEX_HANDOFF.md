...
- `src/stt_pipeline/reference_lookup.py`: DOI/Crossref/OpenAlex/Semantic Scholar lookup planning plus metadata and open PDF cache adapter.
...
1. Add publisher-specific PDF fallback behind `reference_lookup.py`.
2. Add metadata quality scoring and review flags for conflicting reference lookup results.
3. Add skipped integration tests for optional reference/PDF/tools before making them default.
4. Add glossary accumulation from accepted corrections.
...
Use `--lookup-references` to perform live Crossref/OpenAlex/Semantic Scholar metadata lookup and cache open PDF links into `reference_cache/*.pdf`. It also writes `reference_lookup_results.json`; when combined with `--extract-pdfs`, cached PDFs are passed to the figure/table extraction workflow.
